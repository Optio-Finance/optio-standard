...
def owner_account():
    return os.path.join(
        ...
        "../optio_state/current_account.json",
    )
...
def pool():
    return os.path.join(
        os.path.dirname(__file__),
        "../optio_state/current_deployment_info.json",
    )
...
def create_compile_command(name_of_contract, name_of_compiled):
    if not os.path.exists("optio_state/builds"):
        os.makedirs("optio_state/builds")
    if not os.path.exists("optio_state/abis"):
        os.makedirs("optio_state/abis")
    cmd_list = [
        f"starknet-compile",
        f"contracts/{name_of_contract}.cairo",
        f"--output",
        f"optio_state/builds/{name_of_compiled}_compiled.json",
        f"--abi",
        f"optio_state/abis/{name_of_compiled}_abi.json",
    ]
...
def create_deploy_command(name_of_contract, input_list):
    cmd_list = [
        ...
        f"deploy",
        f"--contract",
        f"optio_state/builds/{name_of_contract}_compiled.json",
...
def create_invoke_command(
    ...
    cmd_list = [
        ...
        f"{address}",
        f"--abi",
        f"optio_state/abis/{name_of_contract}_abi.json",
        f"--function",
        f"{function_name}",
        f'--network={os.getenv("STARKNET_NETWORK")}',
        f"--inputs",
...
def write_result_to_storage(result, file_name):
    if not os.path.exists("optio_state"):
        os.makedirs("optio_state")
    with open(f"optio_state/{file_name}.json", "w") as file:
